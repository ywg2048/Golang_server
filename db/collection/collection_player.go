...
func LoadPlayer(clientAccount string, serverAccount string, uid int64) (int32, Player) {
	log.Debug("******LoadPlayer:%v", clientAccount)

	var player Player
	ret := int32(0)

	log.Debug("Before client is %s, serverAccount is %s, uid is %s", clientAccount, serverAccount, uid)
	//测试关数据
	...
	log.Debug("After client is %s, serverAccount is %s, uid is %s", clientAccount, serverAccount, uid)
	...
	if len(clientAccount) <= 0 {
		log.Error("ErrorCode_SysError: lenth(%d) of account is invalid", int(len(clientAccount)))
		ret = int32(cspb.ErrorCode_SysError)
		player.Uid = 0
	} else {
		c := db_session.DB("zoo").C("player")
		log.Debug("*****clientAccount is %v", clientAccount)
		err := c.Find(bson.M{"c_account": clientAccount}).One(&player)

		log.Debug("Find account is %v and player is %v, ", err, player)

		switch err {
		case mgo.ErrNotFound:
			log.Debug("c_account mgo.ErrNotFound...")
			...
			if uid < 0 {
				...
				log.Info("Write player:%v", player)
		...
		case nil:
			if player.Uid != uid {
				// 在设备上就有ID存在，以设备ID为准
				log.Error("在设备上就有ID存在，以设备ID为准.服务器上查的MAYAID is %s, 设备存储 is %d ", player.Uid, uid)
				if uid != 0 {
					log.Error("把服务器存储的id%d更新用户的设备ID%d", player.Uid, uid)
					player.Uid = uid
				}
				...
				log.Debug("获取用户%v ", player)
			}
		default:
			// 因为不是nill，就是出错了
			// 而且错误不是ErrNotFound
			log.Error("ErrorCode_SysError in LoadingPlayer error is: %s", err)
			ret = int32(cspb.ErrorCode_SysError)
		}
	}
...
func ChangeFieldValue(account string, field_name string, add_value int32) int32 {
	log.Debug("account:%s, field_name:%s, add_value:%d",
	...
	if err == mgo.ErrNotFound {
		if add_value < 0 {
			log.Error("no found this attr(%s) , attrvalue's count is 0", field_name)
			return -1
		}
		err = c.Update(bson.M{"_id": account},
			bson.M{field_name: add_value})
		log.Debug("addElementValue new addFieldValue:%s", account)
		return 1
	} else if err != nil {
		log.Error("addElementValue fail err:%v", err)
		return -1
	}
	return 0
}
...
func SetLastSignInTime(account string, last_signin_time int64,
	...
	log.Debug("account:%s, last_signin_time:%d, free_signin_oper_time:%d",
	...
	if err != nil {
		log.Error("SetLastSignInTime fail err:%v", err)
		return -1
	}
...
func SetFieldValue(account string, field_name string, set_value int32) int32 {
	log.Debug("account:%s, field_name:%s, set_value:%d",
	...
	if err != nil {
		log.Error("setElementValue fail err:%v", err)
		return -1
	}
	return 0
}
...
func SetFielsValues(account string, field_names []string, set_values []int32) int32 {
	log.Debug("account:%s, field_names:%v, set_values:%v",
...
func SetLevelValues(account string, levelid int32, stage_level int32, stage_score int32) int32 {
	...
	log.Debug("********account is %v levelid is %v stage_level is %v stage_score is %v", account, levelid, stage_level, stage_score)
	...
	if err != nil {
		log.Error("StageReportHandle fail err:%v", err)
		return -1
	}
	return 0
}
