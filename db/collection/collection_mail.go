...
func AddMail(mail Mail) int32 {
	beego.Debug("mail_info:%v", mail)
	c := db_session.DB("zoo").C("mail")

	mail.MailId = bson.NewObjectId()
	err := c.Insert(&mail)
	if err != nil {
		beego.Error("insert mail fail err:%v", err)
		return -1
	}
	return 0
}
...
func GetMailAll(uid int64) ([]Mail, int32) {
	beego.Debug("uid:%d", uid)
	c := db_session.DB("zoo").C("mail")
	var mail_list []Mail
	err := c.Find(bson.M{"uid": uid}).All(&mail_list)
	if err == mgo.ErrNotFound {
		beego.Debug("mail is not found err:%v, uid:%d",
			err, uid)
		return mail_list, 1
	} else if err != nil {
		beego.Error("query mail fail err:%v, uid:%d",
			err, uid)
		return mail_list, -1
	}
	return mail_list, 0
...
func RemoveMail(mail_id string) int32 {
	beego.Debug("mail_id:%s", mail_id)
	c := db_session.DB("zoo").C("mail")
	err := c.Remove(bson.M{"_id": bson.ObjectIdHex(mail_id)})
	if err == mgo.ErrNotFound {
		beego.Debug("mail is not found err:%v, mail_id:%s",
			err, mail_id)
		return 1
	} else if err != nil {
		beego.Error("remove mail fail err:%v, mail_id:%s", err, mail_id)
		return -1
	}
	return 0
}
