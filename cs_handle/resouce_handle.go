...
func ResourceHandle(
	...
	if req_data.GetType() == int32(3) {
		if req_data.GetDiamond() > 0 {
			_, err := c.Upsert(bson.M{"uid": int32(res_list.GetUid())},
				bson.M{"$inc": bson.M{"flower": req_data.GetFlower(), "diamond": -req_data.GetDiamond()}})
			if err == nil {
	...
	if req_data.GetType() == int32(8) {
		if req_data.GetDiamond() > 0 {
			_, err := c.Upsert(bson.M{"uid": int32(res_list.GetUid())},
				bson.M{"$inc": bson.M{"experience_pool": req_data.GetSolution(), "diamond": -req_data.GetDiamond()}})
			if err == nil {
				beego.Info("药水数量变更成功！")
				issuccess = int32(1)
			} else {
			...
			_, err := c.Upsert(bson.M{"uid": int32(res_list.GetUid())},
				bson.M{"$inc": bson.M{"experience_pool": req_data.GetSolution()}})
			if err == nil {
				beego.Info("药水数量领取成功！")
				issuccess = int32(1)
			} else {
	...
	if req_data.GetType() == int32(9) {
		_, err := c.Upsert(bson.M{"uid": int32(res_list.GetUid())},
			bson.M{"$inc": bson.M{"medal": req_data.GetMedal()}})
		if err == nil {
			beego.Info("勋章变更成功！")
			issuccess = int32(1)
			//mysql排行表的勋章数量变更
